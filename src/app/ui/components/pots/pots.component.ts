...
export class PotsComponent implements OnInit {
  ...
  private productType: string = 'pots';
  potsList$ = new BehaviorSubject<any[]>([]);

  ngOnInit(): void {
    this.productservice.AuthUserGetProduct_PublicProduct(this.productType, this.potsList$);
  }
  PotsAddBasket(outdoorProduct:IList_Product,type:string,name:string){
    this.updatebasketservice.AddBasket(outdoorProduct,type,name);
  }
  addLike(potsProduct: ILike_Item, color: string) {
    this.like.AddOrRemoveUserLike(potsProduct, color);
    this.productservice.AuthUserGetProduct_PublicProduct(this.productType, this.potsList$);
  }
}
